		...
		try (CSVReader reader = new CSVReader(new InputStreamReader(file.getInputStream(), StandardCharsets.UTF_8))) {
			...
			while ((line = reader.readNext()) != null) {
				...
				try {
					...
					MovieBean movie = new MovieBean();
					movie.setCountry(tokens[1].trim());
					movie.setMovieName(tokens[2].trim());
					movie.setApplicant(tokens[4].trim());
					movie.setProdUnit(tokens[5].trim());

					// 安全地解析整數和長整數
					try {
						if (tokens[6] != null && !tokens[6].trim().isEmpty()) {
							movie.setShowTheaterNum(Integer.parseInt(tokens[6].trim()));
						} else {
							movie.setShowTheaterNum(0); // 或者設定預設值
						}
					} catch (NumberFormatException e) {
						System.out.println("第 " + lineNumber + " 行的放映戲院數量格式不正確: " + tokens[5] + "，已設為0");
						movie.setShowTheaterNum(0);
					}

					try {
						if (tokens[7] != null && !tokens[7].trim().isEmpty()) {
							movie.setBoxOffice(Long.parseLong(tokens[7].replace(",", "").trim()));
						} else {
							movie.setBoxOffice(0L);
						}
					} catch (NumberFormatException e) {
						System.out.println("第 " + lineNumber + " 行的票房格式不正確: " + tokens[6] + "，已設為0");
						movie.setBoxOffice(0L);
					}

					try {
						if (tokens[9] != null && !tokens[9].trim().isEmpty()) {
							movie.setSales(Long.parseLong(tokens[9].replace(",", "").trim()));
						} else {
							movie.setSales(0L);
						}
					} catch (NumberFormatException e) {
						System.out.println("第 " + lineNumber + " 行的銷售額格式不正確: " + tokens[7] + "，已設為0");
						movie.setSales(0L);
					}
